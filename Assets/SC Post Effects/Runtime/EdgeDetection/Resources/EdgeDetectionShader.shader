Shader "Hidden/SC Post Effects/Edge Detection" {
	...
	inline float DistanceFade(float depth) {
		float distanceFade = LINEAR_DEPTH(depth) * _DistanceParams.x;
	...
	half4 fragDNormals(Varyings2 i) : SV_Target
	{
		...
		//Opacity
		float3 edgeColor = lerp(original.rgb, _EdgeColor.rgb, _EdgeOpacity * DistanceFade(centerDepth));
	...
	half4 fragRobert(Varyings3 i) : SV_Target
	{
		...
		float depth = LINEAR_DEPTH(centerDepth) * _DistanceParams.x;
